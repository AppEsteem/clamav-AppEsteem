    ...
    switch (type) {
        ...
        case CL_TYPE_MSEXE:
            ...
            if (SCAN_PARSE_PE && ctx->dconf->pe) {
                ...
                // cli_errmsg("cli_magic_scan: cli_scanpe(%s)(%s)\n", ctx->target_filepath, ctx->sub_filepath ? ctx->sub_filepath : "NULL");
                ret                          = cli_scanpe(ctx);
                // cli_errmsg("cli_magic_scan: cli_scanpe(%s)(%s) returned 0x%x\n", ctx->target_filepath, ctx->sub_filepath ? ctx->sub_filepath : "NULL", ret);
                ctx->corrupted_input         = corrupted_input;
            }
